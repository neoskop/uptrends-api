...
describe('Request', () => {
    ...
    describe(':constructor', () => {
        it('should provide defaults to request', () => {
            const req = new Request({ baseUrl: 'http://example.com', username: 'foo', password: 'bar' });
    ...
    describe(':request', () => {
        ...
        it('should append format to url', () => {
            ...
            expect(reqSpy.getCall(1).args[0].url).to.be.equal('/foobar?foo=bar&format=json');
        });
        ...
        it('should provide json', () => {
            ...
            expect(reqSpy).to.have.been.calledWith({ method: 'GET', url: '/foobar?format=json', json: true });
            expect(reqSpy).to.have.been.calledWith({ method: 'POST', url: '/foobar?format=json', json: { post: 'data' }});
            expect(reqSpy).to.have.been.calledWith({ method: 'DELETE', url: '/foobar?format=json', json: true});
            expect(reqSpy).to.have.been.calledWith({ method: 'DELETE', url: '/foobar?format=json', json: { delete: 'data' }});
        });
        
        it('should reject promise on error', done => {
            (req as any).request('GET', '/foobar').then(() => {
        ...
        it('should return json body', done => {
            const RES = { json: 'body' };
            (req as any).request('GET', '/foobar').then((res : any) => {
